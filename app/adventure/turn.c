...
void dotrav( void )
{
  ...
  if ( !mvflag )
    ...
    newloc = rdest;
    if ( dbugflg )
      blimp_printf( "newloc in dotrav = %d\n", newloc );
  }
}
...
void trobj( void )
{
  if ( verb )
    ...
    blimp_printf( "What do you want to do with the %s?\n",
                  probj( object ) );
}
...
void dwarves( void )
{
  ...
  */
  ...
  for ( i = 1; i < DWARFMAX; ++i ) {
    ...
    */
    for ( try = 1; try < 20; ++try ) {
      j = blimp_rand() % 106 + 15; /* allowed area */
  ...
  if ( dflag == 2 )
    ++dflag;
  if ( attack > 1 ) {
    blimp_printf( "%d of them throw knives at you!!\n", attack );
    k = 6;
  }
  else {
    rspeak( 5 );
  ...
  if ( stick <= 1 ) {
    ...
    blimp_printf( "%d of them get you !!!\n", stick );
  oldloc2 = newloc;
  death();
}
/*
